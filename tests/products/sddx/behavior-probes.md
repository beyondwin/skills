...
## 2026-09-13 입력·현재 상태·리뷰 시나리오 (설계)

아래 표는 입력 해석, 원장의 현재 상태 블록, 오류·host 검증·리뷰 지침을 점검하기
위한 **테스트 설계**이며 실제 모델 실행 결과가 아닙니다. 공급자 호출 없이 문구·참조
계약만 `test_contract.py`로 검사하고, 독립 모델 행동 probe와 라이브 실행은 명시적
승인 범위에서만 수행합니다. 이 표의 모든 행은 현재 `not_measured`입니다.
...
현재 상태 블록은 `<!-- sddx:current:start -->`와 `<!-- sddx:current:end -->`
사이만 교체하며, 첫 줄 계획 식별자와 아래의 완료·fix 이력은 그대로 둡니다.
`run.json`은 한 attempt의 프로세스 사실만 담고 승인·완료·리뷰·다음 계획은 원장이
담습니다. 두 기록을 양방향으로 동기화하지 않습니다.
