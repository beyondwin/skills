...
수정 패스는 최대 두 번입니다. 원래 지적의 작은 잔여만 남으면 한 번 더
고치고 그 지적만 다시 봅니다. 최종 판정은 다음 셋 중 하나입니다.
...
이상이 판정을 바꾸지는 않습니다. 마지막 동작이 수리이면 `READY`가 아니고, 열린
`BLOCKER`가 있으면 `BLOCKED`입니다.

한 호출은 발견 최대 1회와 제한된 재검토로 끝납니다. 첫 검토에서 문제가 없고 앞
계획이 고친 파일을 이 계획이 읽지 않으면, 수리와 재검토를 건너뛰고
`READY`입니다. 앞 수정이 이 계획이 읽는 파일을 바꿨으면 문제가 0건이어도
종결 재검토를 합니다. 나눈 계획의 발견은 겹칠 수 있고 수리는 겹치지
않습니다. 종결에는 수리 diff가 필요합니다. 앞 계획이 `BLOCKED`여도 뒤 계획의
발견은 이어집니다. 답이 없는 사용자 결정으로 `BLOCKED`된 계획은 결정이
문서에 들어갈 때까지 검토자를 다시 부르지 않고 같은 질문을 보여 주며, 새
결정이 세 판 연속 나오면 설계로 돌려보냅니다. 권위를 보존하는 수정은 승인
