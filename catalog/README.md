...
The last published catalog identity is `beyondwin-skills` `2.0.0`. That release locks the public `v2.0.0` standalone payloads for `image-workbench` and `korean-writing-editor`. Current `skills/` development, including unpublished `how-it-works`, is not the catalog source.
