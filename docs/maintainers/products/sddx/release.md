...
`4.0.0`이며 `release.toml`이 원본입니다. 태그와 아티팩트는 없습니다.

`4.0.0` 변경은 CHANGELOG의 `## 4.0.0 - 2026-09-19`에 있습니다. 공개 태그를
...
절차입니다. 4.0.0은 이 확인을 다시 하지 않았고 상태는 `not_measured`입니다.
확인 없이 공개 릴리스 완료를 선언하지 않습니다. 이 명령은 제품 소유 경로와
공용 릴리스 코드의 작업 트리가 깨끗할 때만 통과하므로 변경을 커밋한 뒤
실행합니다.
