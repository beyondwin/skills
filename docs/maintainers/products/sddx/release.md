...
소유하며 현재 개발 목표 버전은 `4.0.0`입니다. 아직 출시하지 않았으므로 태그도
아티팩트도 없습니다.
...
4.0.0 변경은 CHANGELOG의 `## 4.0.0 - 2026-09-19`에 둡니다. 릴리스를
준비할 때 이 절을 확정하고 새 빈 `Unreleased` 절을 엽니다.
...
공개 릴리스 완료를 선언하지 않습니다. XHigh 라이브 로딩은 공개 태그 전
절차이며, 이번 4.0.0 변경의 범위가 아닙니다. 이 명령은 제품 소유 경로와 공용
릴리스 코드의 작업 트리가 깨끗할 때만 통과하므로 변경을 커밋한 뒤 실행합니다.
