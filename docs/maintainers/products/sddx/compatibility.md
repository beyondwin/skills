...
현재 제품 버전은 `7.0.1`입니다. 7.0.1, 7.0.0, 6.0.0은 워커 모델 계약을 바꾸지 않습니다.
5.0.0은 워커를 Grok 4.7로 고정하고 `-fast` 변형을 빼며, 아래 표의 워커 칸을
갈아 끼우지 않습니다. 그 칸은 `2.0.0`대 CLI 측정입니다. 4.0.3에서 현재
Cursor `2026.09.15-d2fe57e`와 Grok `1.0.34`로 `run_worker.py` 스모크를 다시
돌렸습니다. 호스트×워커 네 조합 품질 표는 그 스모크로 갈아 끼우지 않습니다.
