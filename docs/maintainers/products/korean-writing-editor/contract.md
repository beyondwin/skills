...
트리거, 모드, 출력, 증거, 테스트 예시, 버전을 함께 유지하세요. 안내문만 고치고
...
문제만 말합니다. `correct`와 `polish`는 규범과 분명히 고쳐야 하는 국소 문법
오류를 교정합니다. 선택적인 가독성과 국소 흐름 개선은 `polish`에서만 합니다.
...
중복 조사나 분명히 잘못된 호응은 두 편집 모드 모두에서 국소 교정합니다.
주어, 높임 수준, 뜻을 추측해야 하는 문법 수정은 hold 규칙을 유지합니다.
...
`correct`와 `polish`의 기본 출력은 편집된 글만입니다. 첫 비공백 문자가 그
본문에 속합니다. `diagnose`의 기본 출력은 소견이며 첫 줄이 문제·등급·hold를
가리킵니다. 다시 쓴 초안, 루브릭, 변경 로그, 점수, 라우팅 영수증, 과정 서술을
붙이지 않습니다. 실질 hold에만 짧은 `확인 필요` 주를 답니다.

제외 near-miss의 no-op는 그 제외 과제를 같은 턴에 수행하지 않습니다. 이미
맞는 국소 표현의 동의어 치환은 `correct`와 `polish` 모두에서 되돌립니다.
