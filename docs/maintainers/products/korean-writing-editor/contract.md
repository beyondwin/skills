...
`correct`와 `polish`의 기본 출력은 고친 글만입니다. 맨 앞 글자가 그 본문이어야
합니다. `diagnose`의 기본 출력은 소견입니다. 첫 줄이 문제, 등급, 또는 hold를
가리킵니다. 다시 쓴 초안, 채점표, 변경 목록, 점수, 라우팅 영수증, “스킬을 쓰는
중” 같은 설명을 붙이지 않습니다. 정말 멈출 때만 짧은 `확인 필요`를 답니다.

편집이 아닌 요청(번역, 초안 등)은 거절만 합니다. 그 턴에서 그 일을 대신 하지
않습니다. 이미 맞는 표현을 비슷한 말로 바꾸면 `correct`와 `polish` 모두에서
되돌립니다.
