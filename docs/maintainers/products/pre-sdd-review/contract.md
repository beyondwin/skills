...
`class`를 유지합니다. 닫힘에서 다른 심각도나 분류의 남은 부분이 나오면 새
ID의 새 기록으로 적습니다. 직전 run이 아직 권위 문서에 없는 사용자 결정
때문에 `BLOCKED`이면 검토자를 부르지 않고 수리하지 않으며 같은 체크포인트를
다시 보여 줍니다.
...
따르고, 결정이 기록되면 이어 검토 조건을 만족할 때 이어 검토합니다. 그 밖의
`blocked`는 입력 게이트를 다시 확인한 뒤 `start`합니다.
