...
현재 지원 호스트는 제품 레지스트리의 `codex`, `claude-code`, `grok`, `cursor`입니다. 이 네 호스트만 로컬 또는 저장소 기준으로 지원합니다. Claude.ai, Cowork, Skills API 업로드, marketplace 게시, 클라우드 동기화는 지원하지 않습니다. 호스트별 페이로드 복사본을 만들지 마세요.
...
| Cursor | `/how-it-works`, 선택적 `@how-it-works` | `~/.agents/skills/how-it-works` |
