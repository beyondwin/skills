...
- 라우팅, 권한, ImageSpec, 핸드오프, inspector 기대는 `tests/image-workbench/cases.json`과 `tests/image-workbench/run.py`가 소유합니다.
- inspector 출력 변경은 `tests/image-workbench/test_inspect_asset.py`, 평가기 full-scope 기대, 공개 문서를 함께 고칩니다. 동작이 바뀌면 SemVer를 올립니다.
- 런타임 inspector는 `skills/image-workbench/scripts/inspect_asset.py`입니다. 테스트는 `tests/image-workbench/test_inspect_asset.py`에 둡니다. 런타임 스크립트에 unittest 스위트를 넣지 마세요.
- inspector는 실제 스킬 루트에서 호출합니다. 저장소 상대 `skills/` 경로로 호출하지 마세요.
- 평가기 또는 inspector 명령·패키지 경로 변경은 `tests/image-workbench/run.py`와 `python3 scripts/verify.py`와 맞춥니다.
