...
제품 버전의 원본은 `skills/<name>/release.toml`입니다. `SKILL.md`의
`metadata.version`은 Agent Skills 소비자가 읽는 배포용 복제 값입니다. CI가 두
값을 함께 바꾸도록 강제합니다. 도구가 두 값을 몰래 맞추지 않습니다.

카탈로그 버전의 원본은 `catalog/release.toml`입니다. 플러그인 매니페스트 버전은
이와 같아야 합니다. 카탈로그 숫자는 스킬 숫자와 맞출 필요가 없습니다.
...
SemVer는 파일 수가 아니라, 밖에서 보이는 제품 계약의 영향을 기준으로
판단합니다.
...
개별 스킬 릴리스가 카탈로그 lock이나 카탈로그 버전을 자동으로 바꾸지 않습니다.
채택은 [카탈로그](catalog.md)의 별도 작업입니다.
...
- 기존 통합 태그 `v2.0.0`은 레거시 기준선으로 보존합니다. 소급해 제품 태그를
  만들지 않습니다.
