...
카탈로그는 이미 공개되어 원격 검증을 통과한 스킬 릴리스를 묶은 플러그인입니다.
마지막 공개 식별은 `beyondwin-skills` `2.0.0`입니다. 이 문서는 무엇을 채택하고,
어떻게 확인하고, 실패하면 어떻게 되돌리는지 적습니다.

- 플러그인 메타데이터는 `catalog/plugin/.codex-plugin/plugin.json`에 있습니다.
  저장소 루트에 두지 않습니다.
- 카탈로그는 제품 계약을 소유하지 않습니다.
...
GitHub Release가 있다고 말하지 마세요. 공개 증명은 원격에서 받은 파일 바이트입니다.
...
허용합니다. `how-it-works`, `pre-sdd-review`, `sddx`는 공개 `v2.0.0`에 포함되지
않았고, 현재 lock에도 없습니다.
...
`legacy-bundle` 입력으로 고정합니다. 지금 `skills/`에 있는 `how-it-works`,
`pre-sdd-review`, `sddx`는 카탈로그 ZIP에 복사되지 않습니다. 공유 버전
번들 빌더 `python3 scripts/build_release.py`는 폐기되었고, 실행하면 실패합니다.

카탈로그 플러그인 메타데이터는 `catalog/plugin/.codex-plugin/plugin.json`에서
읽어 ZIP 루트 `.codex-plugin/plugin.json`으로 복사합니다. 각 standalone ZIP은
`LICENSE.txt`가 있는 최상위 스킬 디렉터리 하나입니다. 테스트, 라이브 실행 도구,
관리자 문서, 캐시, 증거는 스킬 ZIP에 넣지 않습니다.

## archive, extract, checksum

공개 `v2.0.0` 아카이브는 이렇게 만들었습니다.

- 추적된 일반 파일만 씁니다. zip 멤버를 정렬합니다.
- 심볼릭 링크와 특별 파일을 거절합니다.
- 모든 멤버를 `1980-01-01T00:00:00`으로 찍습니다.
- 일반 파일 `0644`와 실행 스크립트 `0755`를 사용했습니다.
...
아카이브를 새 임시 디렉터리에 풉니다. 추출한 한국어·이미지 스킬 파일(payload)과
추출한 inspector로 설치 확인(smoke)을 돌립니다.

## 원격 download

로컬 `dist/`는 공개 증거가 아닙니다. 공개 `v2.0.0`과 이후 릴리스는 원격에서 받은
바이트로 다음을 확인합니다.
...
Archive 현재 트리를 바꾸기 전에 다음 조건이 모두 성립해야 합니다. 하나라도
빠지면 삭제를 막습니다.
...
- 패키징 실패: 새 출력 디렉터리에서 다시 빌드합니다. 부분 결과를 재사용하지
  않습니다.
- 원격 검증 실패: Draft(공개 전 GitHub Release 초안)를 비공개로 유지합니다.
  로컬 성공을 공개 증거로 대체하지 않습니다.
