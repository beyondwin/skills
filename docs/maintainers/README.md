...
이 인덱스는 할 일을 고르는 안내입니다. 사용자 설치 안내는 각 제품 README와 `docs/users/`에 있습니다. 트리가 어떻게 나뉘는지는 [구조](repository/architecture.md)를 보세요.

관리자 문서는 한국어가 원본입니다. 명령, 파일 경로, 계약 식별자는 영어로 둡니다.
`SKILL.md`와 런타임 `references/`는 에이전트 실행 계약이라 영어를 유지합니다.

## 문서 유지

읽는 곳은 둘입니다. 스킬을 고르고 호출할 때는 제품 README를 봅니다. 설치·검증·변경·릴리스는
...
지금 쓰는 문서를 바꾸면 같은 변경에서 digest(문서 지문) 핀과 사실 단언을 맞춥니다. 제품 동작
...
| 제품 등록 | [제품 목록](repository/products-registry.md) |
...
| [제품 목록](repository/products-registry.md) | `products.toml` 스키마와 등록 |
