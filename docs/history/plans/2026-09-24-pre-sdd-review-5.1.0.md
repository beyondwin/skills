...
| L4b' after fix `f54d148`, typo only (default) | Claude Code | 0 dispatches, no `start`, same decision, `Evidence: not_recorded; reason=previous-decision-checkpoint` | 0 dispatches; recorder calls `--version`, `summary` and `show` only, no new run; `BLOCKED` with the same `n = 1` decision (PSDR-001); printed the exact `Evidence:` line and `Anomalies: not_recorded` | pass | 39 s |
| L4c decision recorded in the design (default) | Claude Code | continuation: first dispatch is closure with the prior IDs and the document diff since `sha_end`; review > repair; READY or REVISE | 2 closure dispatches, 0 discovery; the first carried PSDR-001 verbatim plus the design and plan diff since the prior `sha_end`; PSDR-001 partially closed, repaired, then closed; `READY`, review 2 / repair 1, no anomalies | pass | 265 s |
| L2' continuation re-run on `f54d148` (default) | Claude Code | same as L2 | 2 closure dispatches, 0 discovery; prior IDs PSDR-001..004 kept; PSDR-005, from the bounded regression over the diff, was repaired in pass 1; `READY`, review 2 / repair 1, no anomalies; PSDR-002 `repaired` with `repair_pass` null | pass | 173 s |
