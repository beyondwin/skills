...
말 뜻: 픽스처는 미리 만들어 둔 검사 예시입니다. smoke는 실제로 한 번 돌려 본 기록입니다. `not_measured`는 아직 이 환경에서 확인하지 않음, `historical-unbound`는 예전 기록이라 지금 실행 증거가 아님, `current-bounded`는 버전과 hash만 묶였다는 뜻입니다.

```bash
python3 scripts/verify.py
```

인자 없이 돌리면 모든 단계를 돌립니다. 단계는 아래 순서입니다. 첫 실패에서 멈춥니다.
