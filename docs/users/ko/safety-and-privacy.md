...
`pre-sdd-review`는 로컬 설계, 구현 계획, 참조된 ADR, 저장소 파일을 읽습니다. 기본 모드에서는 확인된 설계와 계획만 수정합니다. 사용자 문서를 전송하거나 지속 저장하거나 저장소 소유 테스트 픽스처로 수집하지 않습니다. 명시적인 외부 요청 없이는 구현이나 SDD를 시작하지 않습니다.
